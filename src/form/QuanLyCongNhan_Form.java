...
//		dialogThemCapNhat();
	...
	private void dialogThemCapNhat() {
		...
		dl_ThemCapNhatCN.setSize(750, 650);
		...
		lbl_NgayCongTac.setBounds(20, 360, 120, 25);
		...
		dateChooser_ngayCT.setBounds(148, 360, 157, 25);
		...
		cb_PhanXuong.setBounds(148, 410, 157, 25);
		...
		cb_TayNghe.setBounds(148, 460, 157, 25);
		...
		lblNewLabel_5.setBounds(315, 360, 11, 20);
		...
		dl_ThemCapNhatCN.add(panel_Center, BorderLayout.CENTER);
		...
		dl_ThemCapNhatCN.add(panel_ChucNang, BorderLayout.SOUTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnThemCongNhan)) {
			if(validData()) {
				...
				cn_bus.create(cn);
				dl_ThemCapNhatCN.setVisible(false);
				model.setRowCount(0);
				docDuLieuTuDataVaoTable();
		...
		if (o.equals(btnTimKiem)) {
			...
			if (ma.equals("Nhập mã công nhân cần tìm VD: CN0001")) {
				...
				if (cn!= null) {
					model.setRowCount(0);
					model.addRow(new Object[] { ma, cn.getHoTen(), cn.isPhai() ? "Nam" : "Nữ", cn.getNgaySinh(),
							px_bus.getdsPXtheoID(cn.getPhanXuong().getIdPhanXuong()).getTenPhanXuong(), cn.getTayNghe(),
							cn.getEmail(), cn.getSoDienThoai(), cn.getcCCD()
