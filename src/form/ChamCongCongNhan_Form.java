...
public class ChamCongCongNhan_Form extends RoundPanel implements ActionListener {
	...
	private void initPanel() {
		...
		gl_panelCenter_1.setHorizontalGroup(gl_panelCenter_1.createParallelGroup(Alignment.LEADING)
				.addComponent(panel_2, GroupLayout.DEFAULT_SIZE, 527, Short.MAX_VALUE)
				.addComponent(panel_3, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));
		gl_panelCenter_1.setVerticalGroup(gl_panelCenter_1.createParallelGroup(Alignment.LEADING)
		...
		}, new String[] { "ID", "H\u1ECD v\u00E0 t\u00EAn", "Ca l\u00E0m", "Ph\u00E2n x\u01B0\u1EDFng",
...
//        scrollPane.setColumnHeaderView(panel_1);
		...
		btnCapNhat.setIcon(new ImageIcon(ChamCongCongNhan_Form.class.getResource("/icon/update.png")));
		...
		cboCaLam.setBorder(new EmptyBorder(0, 0, 0, 0));
		cboCaLam.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		...
		textNgayChamCong.setBounds(1118, 5, 93, 35);
		...
		cboPhanXuong.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		cboPhanXuong.setFont(new Font("SansSerif", Font.PLAIN, 15));
		cboPhanXuong.setBounds(856, 5, 117, 37);
		cboPhanXuong.setBackground(Color.WHITE);
...
//        btnLamMoi.setIcon(new ImageIcon(ChamCongCongNhan_Form.class.getResource("/icon/reset.png")));
		...
		btnLamMoi.setBounds(311, 2, 191, 40);
		panelNorth.add(btnLamMoi);

		JLabel lblNewLabel_3_6 = new JLabel("Ca làm:");
		lblNewLabel_3_6.setFont(new Font("SansSerif", Font.PLAIN, 15));
		lblNewLabel_3_6.setBounds(537, 6, 51, 34);
		panelNorth.add(lblNewLabel_3_6);

		JLabel lblNewLabel_3_7 = new JLabel("Phân xưởng:");
		lblNewLabel_3_7.setFont(new Font("SansSerif", Font.PLAIN, 15));
		lblNewLabel_3_7.setBounds(746, 5, 93, 34);
		panelNorth.add(lblNewLabel_3_7);
		...
		// Đưa dữ liệu công nhân lên label khi click vào 1 dòng
		...
		// Xử lý sự kiện tìm kiếm khi nhập vào ô tìm kiếm
		...
		// Xử lý sự kiện khi thay đổi giá trị của 1 dòng chấm công
		tableChamCong.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				int row = tableChamCong.getSelectedRow();
				if (currentRowChamCong != -1 && currentRowChamCong != row) {
					if (capNhatSoLuongHoanThanh(currentRowChamCong)) {
		...
		// Đăng ký sự kiện cho combobox Phân xưởng
		cboPhanXuong.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				kiemTraComboBoxCaLam();
			}
		...
		// Lấy dữ liệu cbo Phân Xưởng
		duaDuLieuPhanXuongLenCBO();

		// Lấy và hiển thị danh sách chấm công
		locDuLieuChamCongTheoNgay();
		// lấy và hiển thị danh sách công nhân
		layDanhSachPhanCong();
	...
	private void docDuLieuLenTablePhanCong(ArrayList<CongDoanPhanCong> list) {
		...
		for (CongDoanPhanCong congDoanPhanCong : list) {
			dm.addRow(new Object[] { congDoanPhanCong.getCongNhan().getIdCongNhan(),
					...
					congDoanPhanCong.getCongNhan().getPhanXuong().getTenPhanXuong(),
					congDoanPhanCong.getCongDoanSP().getSanPham().getTenSanPham(),
	...
	private void locPhanCongTheoHoTen() {
		...
		listPhanCong = new ArrayList<CongDoanPhanCong>();
		for (CongDoanPhanCong congDoanPhanCong : temp) {
			if (congDoanPhanCong.getCongNhan().getHoTen().trim().toUpperCase()
					.contains(txtTimKiem.getText().trim().toUpperCase())) {
				listPhanCong.add(congDoanPhanCong);
			}
		}
	}
	...
	private void docDuLieuLenTableChamCong(ArrayList<BangChamCongCongNhan> listChamCong) {
		...
		for (BangChamCongCongNhan bangChamCongCongNhan : listChamCong) {
			dm.addRow(new Object[] { bangChamCongCongNhan.getCongDoanPhanCong().getCongNhan().getIdCongNhan(), bangChamCongCongNhan.getCongDoanPhanCong().getCongNhan().getHoTen(),
					...
					bangChamCongCongNhan.getCongDoanPhanCong().getSoLuongSanPhamDuocGiao(),
					bangChamCongCongNhan.getSoLuongHoanThanh() });
		}
	...
	private void locDuLieuChamCongTheoNgay() {
		listChamCong = bangChamCongCongNhan_BUS.getDanhSachChamCongTheoNgay(date.getDayOfMonth(), date.getMonthValue(),
				date.getYear());
		docDuLieuLenTableChamCong(listChamCong);
	}
	...
	private void duaDuLieuPhanXuongLenCBO() {
		PhanXuong_BUS phanXuong_BUS = new PhanXuong_BUS();
		ArrayList<PhanXuong> listPX = phanXuong_BUS.getDanhSachPhanXuong();
		String[] item = new String[listPX.size() + 1];
		int i = 0;
		for (PhanXuong phanXuong : listPX) {
			item[i] = phanXuong.getTenPhanXuong();
			i++;
		}
		item[listPX.size()] = "Tất cả";
		cboPhanXuong.setModel(new DefaultComboBoxModel<String>(item));
		cboPhanXuong.setSelectedIndex(listPX.size());
	}

	private void locTheoPhanXuong() {
		if (cboPhanXuong.getSelectedItem().equals("Tất cả")) {
			return;
		}
		ArrayList<CongDoanPhanCong> temp = new ArrayList<CongDoanPhanCong>();
		for (CongDoanPhanCong congDoanPhanCong : listPhanCong) {
			if (congDoanPhanCong.getCongNhan().getPhanXuong().getTenPhanXuong()
					.equals(cboPhanXuong.getSelectedItem().toString())) {
				temp.add(congDoanPhanCong);
			}
		}
		listPhanCong = new ArrayList<CongDoanPhanCong>();
		listPhanCong.addAll(temp);
	}
	...
	public void chamCongHangLoat() {
		if ((JOptionPane.showConfirmDialog(this, "Bạn có muốn chấm công cho toàn bộ công nhân?", "Hỏi nhắc!",
			...
			for (CongDoanPhanCong congDoanPhanCong : listPhanCong) {
				...
				BangChamCongCongNhan bangChamCongCongNhan = new BangChamCongCongNhan(idChamCong, ngayChamCong,
						soLuongHoanThanh, congDoanPhanCong, heSoNgayLam);
				bangChamCongCongNhan_BUS.themChamCong(bangChamCongCongNhan);
				congDoanPhanCong.setSoLuongConLai(soLuongConLai - soLuongHoanThanh);
				congDoanPhanCong_BUS.capNhatSoLuongConLai(congDoanPhanCong);
			}
	 ...
	 */
	private boolean capNhatSoLuongHoanThanh(int row) {
		if (row != -1) {
			...
			try {
				...
				if (soLuongHoanThanh > (bangCC.getSoLuongHoanThanh()
					...
					if (JOptionPane.showConfirmDialog(this, "Bạn có muốn thay đổi không?", "Hỏi nhắc",
						...
						congDoanPhanCong.setSoLuongConLai(congDoanPhanCong.getSoLuongConLai()
								- (soLuongHoanThanh - bangCC.getSoLuongHoanThanh()));
	...
	/**
	 * Cập nhật số lượng hoàn thành cho công nhân khi nhấn nút chấm công
	 */
